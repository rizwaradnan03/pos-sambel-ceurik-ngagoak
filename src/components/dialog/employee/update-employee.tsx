...
import { UseFetchFindOneEmployeeById } from '@/hooks/api/employee/findOneById'
...
const UpdateEmployee = ({ employeeId, setIsDoneUpdatingEmployee }: { employeeId: string, setIsDoneUpdatingEmployee: (value: boolean) => void }) => {
    ...
    const handleCreateProduct = async () => {
        ...
        try {
            ...
            setIsDoneUpdatingEmployee(true)
    ...
    const fetchEmployee = async () => {
        try {
            const {data}: {data: ISEmployee} = await UseFetchFindOneEmployeeById({id: employeeId})
            setName(data.name)
            setPhoneNumber(data.phoneNumber)
            setRole(data.role)
            setSalaryPerDay(formatPrice({value: data.salaryPerDay.toString()}))
            setTransport(formatPrice({value: data.transport.toString()}))
        } catch (error: any) {
            toast.error(error.message)
        }
    }
