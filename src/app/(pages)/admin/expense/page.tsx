...
const page = () => {
  ...
  const { data: dataExpense } = UseFetch<ISExpense[]>({
    key: "inventoryExpense", dependencies: [], refetchDependencies: [{ stateValue: isDoneCreatingExpense, stateSetter: setIsDoneCreatingExpense }], apiFunction: async () => {
          ...
          <TableBody>
            {dataExpense?.map((expense, index) => (
              <TableRow key={expense.id}>
                ...
                <TableCell>
                  Rp{formatPrice({ value: expense?.amount ? expense.amount.toString() : "0" })}
                </TableCell>
